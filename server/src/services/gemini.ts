...
export async function transcribeAudio(audioAsBase64: string, mimeType: string) {
  const response = await gemini.models.generateContent({
    ...
    contents: [
      {
        text: "Transcreva o áudio para português do Brasil. Seja preciso e natural na transcrição. Mantenha a pontuação adequada e divida o texto em parágrafos quando apropriado.",
      },
      {
        inlineData: {
          mimeType,
...
export async function generateAnswer(
  ...
  const prompt = `
    ...
    - Se for citar o contexto, utilize o termo "Conteúdo da aula", mas preferencialmente não cite o contexto, apenas responda a pergunta diretamente.;
  `.trim();
